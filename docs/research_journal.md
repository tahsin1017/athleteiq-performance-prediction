...
Stress Level — approximately 0.1009

Occupation_Doctor — approximately 0.0356

Heart Rate — approximately 0.0340

Daily Steps — approximately 0.0090
...
Without Sleep Duration + Stress Level + Sleep Disorder
...
Without Sleep Duration + Stress Level
...
Profiles lost after removing Sleep Duration: 33

Pre-sleep profiles with multiple target values: 0

Removing Sleep Duration reduced the number of unique predictor profiles from 132 to 99, but no identical pre-sleep predictor profile mapped to multiple target values.
...
Maximum exact predictor-profile overlap between training and validation folds was:
...
Each fold used a separately cloned pipeline before fitting and prediction.
...
Full-feature Mean R²: 0.9481

Pre-sleep Mean R²: 0.9222

R² change: -0.025848

Full-feature Mean MAE: 0.0865

Pre-sleep Mean MAE: 0.1091

MAE change: +0.022587

RMSE change: +0.062996

Removing same-night Sleep Duration reduced Random Forest performance, with Mean R² decreasing by approximately 0.0258.
...
The relatively modest decline in performance suggests that substantial predictive signal remains in variables such as stress, physical activity, age, cardiovascular measures, occupation, BMI category, and sleep-disorder status.

The result also shows why feature availability must be defined before final model selection. A predictor can be statistically useful but still be inappropriate if it would not exist at the intended prediction time.

High predictive performance should not be interpreted as evidence of external generalization.

Methodological Improvement

Day 11 used the stricter pre-sleep profile groups to define the shared folds for the direct Full vs Pre-sleep comparison.

This was important because removing Sleep Duration changed the profile structure from 132 unique full-feature profiles to 99 unique pre-sleep profiles. Comparing the two scenarios on independently generated folds would mix the effect of feature removal with the effect of different train-validation assignments.

Using the same pre-sleep-defined folds therefore produced a fairer estimate of the effect of removing Sleep Duration.

Limitations

The dataset contains only 374 rows.

The pre-sleep feature set contains only 99 unique predictor profiles.
...
Group-aware validation prevents exact profile overlap but cannot establish causal validity.
...
notebooks/09_prediction_scenario_analysis.py

Final Verification

The final Day 11 script completed successfully.

Verified outputs:

Dataset rows: 374

Full-feature profiles: 132

Pre-sleep profiles: 99

Maximum profile overlap: 0

Random Forest full-feature R²: 0.9481

Random Forest pre-sleep R²: 0.9222

Random Forest R² change: -0.025848

All four expected report files were present.

The expected figure file was present.

Zed reported No problems after the final code cleanup.

The final verification ended with:

Day 11 analysis completed successfully.

Reflection

Day 11 showed that a model can remain highly predictive even after removing a feature that appears dominant under feature-importance analysis.

The Random Forest lost more performance than the linear models when Sleep Duration was removed, but it still achieved strong group-aware performance.

The experiment also reinforced an important machine-learning principle: evaluation should reflect the actual prediction setting. A model should not be judged only by how accurately it predicts using every available variable; it should also be tested using only information that would realistically exist at the time the prediction is made.

Questions Raised

Which remaining predictors would truly be available before sleep?

Should Sleep Disorder be retained in a deployment-oriented feature set?

Should blood pressure be considered practical for the intended use case?

How stable is the pre-sleep model across repeated group-separated splits?

Will Random Forest remain the strongest model under a stricter deployment feature set?

How much performance remains after removing both Sleep Duration and other potentially unavailable variables?

When should hyperparameter tuning finally be introduced?

Next Steps

Define a stricter deployment-oriented feature set.

Reconsider Sleep Disorder, blood pressure, and other potentially contemporaneous or clinical predictors.

Compare the pre-sleep feature set with the stricter deployment feature set.
